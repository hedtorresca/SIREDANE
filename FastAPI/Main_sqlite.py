...
from FastAPI.utils.utils_sqlite import (
...
app = FastAPI(
    title="SIRE - Sistema de Identificación Estadística (SQLite)",
    description="API para generar IDs estadísticos para pseudonimización de datos - Versión SQLite para pruebas",
    version="1.0.0"
)
...
@app.get("/")
def read_root():
    return {"message": "SIRE - Sistema de Identificación Estadística (SQLite)", "database": "SQLite"}
...
def generar_id_empresas_cc(request: EmpresaCCRequest):
    """
    Genera un ID estadístico para una empresa con tipo de documento CC.
    
    Si la empresa tiene tipo de documento CC:
    1. Busca si la persona existe en raw_obt_personas
    2. Si existe, usa ese ID para la empresa
    3. Si no existe, crea la persona y usa ese ID para la empresa
    """
    try:
        ...
        if id_empresa_existente:
            return IDResponse(
                ...
                mensaje="Empresa ya existe en el sistema",
                tipo_entidad="01" if id_empresa_existente.startswith("01") else "02",
        ...
        if id_persona_existente:
            ...
            id_persona_existente = id_persona_existente["id_estadistico"]
